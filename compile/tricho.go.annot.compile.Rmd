...
blast <- read_tsv("tricho_annot_diamond.tsv", col_names = FALSE) # this is the output from DIAMOND blastx
head(blast)
...
uniprot_2 <- read_tsv("uniprot_blast.tsv", col_names = TRUE)
...
head(Uniprot_results)
nrow(filter(Uniprot_results, grepl("GO:",go_ids))) #Genes with GO terms #2001
```

**Blast2GO** 
This first one is from what I did in Omics Box (IPS, GO mapping and annotation)
...
B2G_results <- read_csv("omicsbox_table.csv", col_names = TRUE)
head(B2G_results)
...
dim(B2G_results_clean) # 15971
nrow(filter(B2G_results_clean, grepl("GO:",concat_IDs))) #6704 accessions total (same as GO mapping) 
head(B2G_results_clean)
tail(B2G_results_clean)
dim(B2G_results_clean) # 15971   11
...
**Blast2GO** 
This second one is from what Nina did (IPS on the command line, then blast2go)
...
```{r blast2go_merge, eval = TRUE, message = FALSE, warning = FALSE}
B2G_results2 <- read_csv("NY_blast2go_table_Tricho.csv", col_names = TRUE)
head(B2G_results2)
B2G_results2 <- B2G_results2[,c(3:5, 7:8,10:11, 15:16)] # select the relevant columns
head(B2G_results2)
colnames(B2G_results2) <- c("seqName",  "tophit", "length",  "eValue", "simMean", "GO_IDs", "GO_names", "InterPro_GO_IDs", "InterPro_GO_names") # rename the relevant columns
B2G_results2$GO_IDs <- gsub(" ", "", B2G_results2$GO_IDs) # remove unwanted spaces
B2G_results2$InterPro_GO_IDs <- gsub(" ", "", B2G_results2$InterPro_GO_IDs) # remove unwanted spaces
head(B2G_results2)
B2G_results2$InterPro_GO_IDs <- gsub("noGOterms", "NA", B2G_results2$InterPro_GO_IDs) # replace with NA
B2G_results2$InterPro_GO_names <- gsub("no GO terms", "NA", B2G_results2$InterPro_GO_names) # replace with NA
B2G_results2$InterPro_GO_IDs <- gsub("noIPSmatch", "NA", B2G_results2$InterPro_GO_IDs) # replace with NA
B2G_results2$InterPro_GO_names <- gsub("no IPS match", "NA", B2G_results2$InterPro_GO_names) # replace with NA
head(B2G_results2)
dim(B2G_results2) #15833
nrow(filter(B2G_results2, grepl("GO:",GO_IDs))) # 2106 GO
nrow(filter(B2G_results2, grepl("GO:",InterPro_GO_IDs))) # 5244 IPS
B2G_results_clean2 = B2G_results2 %>% 
  unite(concat_IDs, c(GO_IDs, InterPro_GO_IDs), sep = ";", remove = FALSE) %>%
  unite(concat_ontology, c(GO_names, InterPro_GO_names), sep = ";", remove = FALSE)  # merge GO and IPS IDs
head(B2G_results_clean2)
B2G_results_clean2$concat_ontology <- gsub("; ", ";", B2G_results_clean2$concat_ontology) # remove unwanted space after ;
head(B2G_results_clean2$concat_ontology)
B2G_results_clean2$concat_IDs <- sapply(B2G_results_clean2$concat_IDs, function(x) paste(unique(unlist(str_split(x,";"))), collapse = ";")) # remove duplicated IDs after merge
B2G_results_clean2$concat_ontology <- sapply(B2G_results_clean2$concat_ontology, function(x) paste(unique(unlist(str_split(x,";"))), collapse = ";")) # remove duplicated terms after merge
head(B2G_results_clean2)
B2G_results_clean2$concat_IDs <- gsub("F:", "", B2G_results_clean2$concat_IDs) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_ontology <- gsub("F:", "", B2G_results_clean2$concat_ontology) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_IDs <- gsub("P:", "", B2G_results_clean2$concat_IDs) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_ontology <- gsub("P:", "", B2G_results_clean2$concat_ontology) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_IDs <- gsub("C:", "", B2G_results_clean2$concat_IDs) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_ontology <- gsub("C:", "", B2G_results_clean2$concat_ontology) # remove unwanted characters for downstream processing
dim(B2G_results_clean2)
nrow(filter(B2G_results_clean2, grepl("GO:",concat_IDs))) #1698 accessions total (same as GO mapping) 
head(B2G_results_clean2)
tail(B2G_results_clean2)
dim(B2G_results_clean2) # 15833   11
B2G_results_clean2$concat_IDs <- gsub("NA;", "", B2G_results_clean2$concat_IDs) # remove unwanted characters for downstream processing
B2G_results_clean2$concat_ontology <- gsub("NA;", "", B2G_results_clean2$concat_ontology) # remove unwanted characters for downstream processing
```

**Compile blast2go Results**

```{r compile_uniprot, eval = TRUE, message = FALSE, warning = FALSE}
b2g_results_combine <- bind_rows(B2G_results_clean, B2G_results_clean2) # combines blast2go results
b2g_results_combine <- unique(b2g_results_combine) # I think mine were already all unique
b2g_results_combine$GO_IDs <- gsub(" ", "", b2g_results_combine$GO_IDs) # remove unwanted spaces
head(b2g_results_combine)
dim(b2g_results_combine) #31804
b2g_results_combine$concat_ontology <- gsub(" \\[.*?\\]", "", b2g_results_combine$concat_ontology) #remove unwanted information within brackets to streamline compilation later on
b2g_results_combine$concat_ontology <- gsub("; ", ";", b2g_results_combine$concat_ontology) #remove unwanted characters
head(b2g_results_combine)
nrow(filter(b2g_results_combine, grepl("GO:",GO_IDs))) #Genes with GO terms #4328
```
...
nrow(Uniprot.GOterms) # 5742 rows / terms
head(Uniprot.GOterms)
B2G_GO <- dplyr::select(b2g_results_combine, tophit, concat_IDs)
...
tricho_annot <- left_join(blast, b2g_results_combine, by="seqName")
head(tricho_annot)
tail(tricho_annot)
dim(tricho_annot) #28694

# tricho_annot <- select(tricho_annot, seqName, tophit.x, length.x, evalue, bitscore, simMean, concat_IDs, concat_ontology) #this wasn't working for me so im trying this below
...
# tricho_annot <- select(tricho_annot,-c("concat_IDs", "concat_ontology", "gene_ontology", "go_ids", "length", "uniprotkb_entry", "status", "protein_names", "gene_names", "organism"))
...
dim(tricho_annot) # 29188     8
head(tricho_annot)
median(tricho_annot$eValue) #7.17e-48
median(tricho_annot$bitscore) #165
df <- tricho_annot %>%
...
kegg <- read_csv("filtered_tricho_kofamscan.csv", col_names = TRUE) # read in KEGG annotation (this is based on ...)
head(kegg)
dim(kegg) #2338
...
head(KO_annot)
dim(KO_annot) # 13836, only the genes that have GO annotation
...
#some of the text looks a little weird KK

dim(terms_KO_df) # 1323 x 6
head(terms_KO_df)
...
dim(final_KO_annot) # final annotation worksheet for Crocosphaera files where duplicated KOs are merged; also the "nones" are replaced with blastx accession ids (11083 x 5 dim)
...
head(final_KO_annot)
tail(final_KO_annot)
dim(final_KO_annot) #11083, same number as above
